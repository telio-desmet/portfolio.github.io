...
<head>
    ...
    <title>Chat avec Gemma 3-4B</title>
    <style>
        body {
        ...
        fetch('/test')
            ...
            .then(data => {
                if (data.model_available) {
                    statusDiv.textContent = '✅ Gemma 3-4B connecté et prêt';
                    statusDiv.className = 'status success';
                } else {
                    statusDiv.textContent = '⚠️ Gemma 3-4B non disponible - Mode test activé';
...
try:
    model = OllamaLLM(model="gemma3", temperature=0.1, max_tokens=1000)
